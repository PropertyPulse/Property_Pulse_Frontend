...
const LandRegistration = () => {
    ...
    const [selectedInsuranceDocuments, setSelectedInsuranceDocuments] = useState([]);
    ...
    const inputs = [
        ...
        // properties for crops listing input field
        {
            id: 8,
            name: 'crops',
            type: 'textarea',
    ...
    const handleAddProperty = async (e) => {
        ...
        try {
            ...
            form.append('land_size', parseFloat(values.landSize));
            form.append('have_crops', values.haveCrops);
            form.append('crops', values.crops);
            form.append('special_facts', values.specialFacts);
            form.append('want_insurance', wantInsurance);
            form.append('property_owner_email', auth.user);
            form.append('property_images', selectedImages[0]);
            form.append(`insurance_documents`, selectedInsuranceDocuments[0]);
            // Append images and documents as needed
            // selectedImages.forEach((image, index) => {
            //     form.append(`property_images[${index}]`, image);
            // });
            // selectedInsuranceDocuments.forEach((document, index) => {
            //     form.append(`insurance_documents[${index}]`, document);
            // });

            // Use Axios to send the POST request with the FormData object
            const response = await axiosPrivate.post(
                "/api/v1/property/addNewLandProperty",
                form,
                {
                    headers: {
    ...
    const handleInsuranceDocumentsSelected = (documents) => {
        setSelectedInsuranceDocuments(documents);
    };
